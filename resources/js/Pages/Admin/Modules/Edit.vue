...
const form = useForm({
    ...
    teacher: props.module.teacher,
    tag: props.module.tag,
    video: props.module.video,
    parent_module_id: props.module.parent_module_id,
    order: props.module.order,
		references: JSON.parse(props.module.references) ?? {title: '', content: ''},
		questions_references_title: props.module.questions_references_title
});
...
const selectedCourses = props.courses.find((el) => el.id == form.course_id)

const teacher = reactive(JSON.parse(form.teacher));
...
const loadModules = async (course) => {
    try {
        ...
        if(form.parent_module_id) {
            const {id, title} = modules.value.find((el) => el.id == form.parent_module_id)
...
triggerDropdown(selectedCourses, 'courses');
</script>

<template>
    <PageLayout>

        <Head title="Editar un modulo" />
