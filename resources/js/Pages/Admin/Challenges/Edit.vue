...
const form = useForm({
    ...
    legal: props.challenge.legal,
    challenge_type: props.challenge.challenge_type,
    score: props.challenge.score,
    date_from: props.challenge.date_from,
    date_to: props.challenge.date_to,
    participation: ''
});

let url = ref(form.image)
let participation = reactive(JSON.parse(props.challenge.participation));

const submit = () => {
    form.participation = JSON.stringify(participation)
    form.put(route('admin.challenges.update', props.challenge.id));
};
...
const updateDropdown = (opt, propertyName) => {
    type_dropdown.value = opt
    form.challenge_type = propertyName
}

updateDropdown(props.challenge_types[form.challenge_type], form.challenge_type);
            ...
            <form @submit.prevent="submit" class="max-w-xl mx-auto mt-10">
                ...
                <div class="grid md:grid-cols-2 md:gap-x-5">
                    <div class="mt-7">
                        ...
                        <Dropdown>
                            ...
                            <template #content>
                                <Option v-for="(opt, propertyName) in props.challenge_types" :key="propertyName"
                                    @click="triggerDropdown(opt, propertyName)">
                                    {{ opt }}
                                </Option>
                            </template>
                        </Dropdown>
                ...
                <div class="grid md:grid-cols-2 md:gap-x-5">
                    <div class="mt-7">
                        ...
                        <VueDatePicker id="date_from" locale="es" class="mt-1 block w-full" v-model="form.date_from"
                            placeholder="Fecha desde" text-input cancelText="Cancelar" selectText="Aceptar"
                            month-name-format="long" format="dd/MM/yyyy" :enable-time-picker="false" auto-apply
                    ...
                    <div class="mt-7">
                        ...
                        <VueDatePicker id="date_to" locale="es" class="mt-1 block w-full" v-model="form.date_to"
                            placeholder="Fecha hasta" text-input cancelText="Cancelar" selectText="Aceptar"
                            month-name-format="long" format="dd/MM/yyyy" :enable-time-picker="false" auto-apply
                            model-type="yyyy/MM/dd"
                        />
                ...
                <div class="mt-7">
                    ...
                    <a :href="form.legal" _target="blank">Enlace</a> <br/>
                    <input
                        type="file"
